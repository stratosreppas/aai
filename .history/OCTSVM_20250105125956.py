...
class OCTSVM():
    ...
    def normalize_data(self, X):
        # Normalize the features by x-xmin/xmax-xmin for each feature, include the exception of when the max and min are the same
        return (X - X.min(axis=0)) / (X.max(axis=0) - X.min(axis=0)) if X.max(axis=0) != X.min(axis=0) else X
