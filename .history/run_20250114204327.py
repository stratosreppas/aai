...
from utils.datasets import import_dataset, kfolds
from utils.noise import flip_labels
from models.OCTSVM.OCTSVM import OCTSVM
from models.OCT.tree.oct import optimalDecisionTreeClassifier
import csv

datasets = ['Australian', 'Wholesale']
tree_types = ['OCT']
flip_percentages = [0]
max_depth = 3
repetitions = 4
        ...
        for tree_type in tree_types:    
            for percentage in flip_percentages:
                for fold, (X_train, X_test, y_train, y_test) in enumerate(kfolds(X, y, folds)):
                    # print(y_train)
                    y_flipped = flip_labels(y_train, percentage)
                    # print('Flipped labels:', y_flipped)
                    print('Noise level:', accuracy_score(y_train, y_flipped))
                    y_train = y_flipped
                    for i in range(-3, 0):
                        c1 = 10**i
                        if tree_type == 'CART':
