    ...
    def fit(self, X, y):
        ...
        for t in range(T):
            self.model.addConstr(
                quicksum(w[t, j] * w[t, j] for j in range(p)) <= 4 * delta**2, 
            ...
            if(t != 0):
                ...
                self.model.addConstr(
                    quicksum(w[t, j] * w[t, j] for j in range(p)) <= M**2 * d[t]**2,
        ...
        self.model.Params.TimeLimit = 15
        self.model.Params.OutputFlag = 0
        
        self.model.optimize()
        self.model.write('octsvm.lp')
    ...
    def predict(self, X_test):
        ...
        if self.model.status == GRB.OPTIMAL or self.model.status == GRB.TIME_LIMIT:
            ...
            for v in self.model.getVars():
                ...
                if v.varName[0] == 'w':
                    i = int(v.varName.split('_')[1]) - 1
                    t= int(v.varName.split('_')[2])-1
                    w[i, t] = v.x
            # build the tree
            tree = {}
            node = 0
