...
class OCTSVM():
    ...
    def fit(self, X, y):
        ...
        beta = {(i, t, j): self.model.addVar(lb=-GRB.INFINITY, name=f"beta_{i}_{t}") for t in range(T) for i in range(N) for j in range(p)}
        ...
        d = {t: self.model.addVar(lb=0, name=f"d_{t}") for t in range(T)}
